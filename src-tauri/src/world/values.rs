...
impl Values {
    pub fn store(
        ...
        Self {
            ...
            texture: format!("/assets/map/{}", texture),
        }
    }
    ...
    pub fn get(value: &str) -> Self {
        VALUES.clone().into_iter().find(|v| v.value == value).unwrap()
    }

    pub fn get_value(value: &str) -> (String, String, String, bool) {
        let value = VALUES.clone().into_iter().find(|v| v.value == value).unwrap();
        (value.display_value, value.display_color, value.texture, value.walkable)
    }
