pub mod admin_dashboard {
    ...
    impl AdminModel for AdminStoryLineModel {
        fn name(&self) -> &'static str { "story" }
        fn model(&self) -> &'static str { "Storyline" }
        fn command(&self) -> &'static str { "fetch_storyline" }
    }

    pub struct AdminGameModel;
    impl AdminModel for AdminGameModel {
        fn name(&self) -> &'static str { "game" }
        fn model(&self) -> &'static str { "Game" }
        fn command(&self) -> &'static str { "fetch_games" }
    }

    pub struct AdminCollectionModel;
    impl AdminModel for AdminCollectionModel {
        fn name(&self) -> &'static str { "collection" }
        fn model(&self) -> &'static str { "Collection" }
        fn command(&self) -> &'static str { "load_collections" }
    }
    ...
    impl AdminDashboard {
        ...
        fn export(&self) -> Result<Value, Error> {
            let models: Vec<AdminModelProxy> = self.models.iter().map(|model| AdminModelProxy { model: model.as_ref() }).collect();
